...
/*end force uart*/
  ...
  //  serial_puts("console_setfile Done!\n\r");
   
	gd->flags |= 0;/*GD_FLG_DEVINIT;	*//* device initialization completed */
...
/*#ifndef CFG_CONSOLE_INFO_QUIET*/
	...
	serial_puts ("In:    ");
	if (stdio_devices[stdin] == NULL) {
		serial_puts ("No input devices available!\n");
	} else {
		serial_printf ("%s\n", stdio_devices[stdin]->name);
	}
	...
	serial_puts ("Out:   ");
	if (stdio_devices[stdout] == NULL) {
		serial_puts ("No output devices available!\n");
	} else {
		serial_printf ("%s\n", stdio_devices[stdout]->name);
	}
	...
	serial_puts ("Err:   ");
	if (stdio_devices[stderr] == NULL) {
		serial_puts ("No error devices available!\n");
	} else {
		serial_printf ("%s\n", stdio_devices[stderr]->name);
	}
#endif /* CFG_CONSOLE_INFO_QUIET */
  ...
  //  serial_puts("Environment variables set!\n\r");
#if 0
	/* If nothing usable installed, use only the initial console */
	if ((stdio_devices[stdin] == NULL) && (stdio_devices[stdout] == NULL))
		return (0);
