...
#define CHECK_RIGHT 0xa5
#define CHECK_WRONG 0x0f

/*
recieve length byte data from uart to memory
*/
void do_loadu_command(cmd_tbl_t *cmdtp, int flag,int argc,uchar* argv[]){
ulong addr,length,end_addr;
uchar temp[4];
ulong sum;
volatile ulong *lptr;
volatile ulong *addrm;
volatile uchar *addrb;
int i;
int is_sum_check;

 /*   printf("argc:%d\n",argc);*/
    if (argc != 2) {puts("Invalid arg. e.g.#loadu memaddr\n");return;}
...
U_BOOT_CMD(
	loadu, 4, 0,	do_loadu_command,
	"loadu   - load binary file over uart\n",
	"[ off ]\n"
	"    - load binary file over uart"
	" with offset 'off'\n"
);
