...
static ulong led_reload;
static ulong led_status;

void do_irq (struct pt_regs *pt_regs)
{
    ulong temp_u32;

    printf ("IRQ Handled!  INT Source : %d\n",inl(INT_STAT));

    temp_u32 = inl(INT_IVAR);  // IRQ Handled
     enable_interrupts();   // enable CPU interrupts
     outl(1,INT_LEVACK);    // Timer1 IRQ LEVEL 0 ACK 
...
{
    ulong now = 0;/*READ_TIMER; */  	/* current tick value */
