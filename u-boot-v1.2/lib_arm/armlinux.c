...
void Nand_relocate(  unsigned long rd_start,unsigned char * write_start,unsigned long end_addr)
{
unsigned long temp,i;

while(rd_start<end_addr){

//temp = (unsigned long)rd_start;
if ((rd_start&0x100)==0) Nand_FLASH_CMD_ADDR = 0x00;
else Nand_FLASH_CMD_ADDR = 0x01;

Nand_FLASH_ADDR = (rd_start&0xff);
...
for (i=0;i<256;i++){	
	*write_start = Nand_FLASH_DATA_ADDR;
	rd_start++;
	write_start++;
	}
}
...
gpio->GPFDAT = 0x6F;*/
    	...
    	int i;
			...
			for(i=0; i<=1000000; i++);
			outl(0x5555,ALPAS3310_GPIO_DATA);    		
			for(i=0; i<=1000000; i++);
			outl(0,ALPAS3310_GPIO_DATA);
			for(i=0; i<=10000; i++);
}
